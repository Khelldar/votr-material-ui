...
export function createService(client: ApolloClient<any>) {
  ...
  export const GetElectionsQuery = gql`query GetElections($ids:[ID!]!){getElections(input:{ids:$ids}){__typename elections{__typename candidates{__typename description id name}description id name status statusTransitions{__typename on status}}}}`
  
  export const ListElectionsQuery = gql`query ListElections{listElections(input:{}){__typename elections{__typename dateUpdated description id name status}}}`
